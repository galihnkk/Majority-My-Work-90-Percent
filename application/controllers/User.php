...
class User extends CI_Controller
{
              ...
              redirect(site_url('daftar'));
                ...
                if($this->Crud_m->tambah_user_detail($data_user_detail))
                {
                  if($this->sendemail($email,$saltid,$username))
                                {
                                    $this->session->set_flashdata('msg','<div class="alert bg-5 text-center">Segera lakukan aktivasi akun brandname dari email anda. Harap merefresh pesan masuk di email Anda.</div>');
  ...
  public function sendemail($email,$saltid,$username){
        // configure the email setting
            ...
            $config = array (
              'protocol'    => 'smtp',
              'smtp_host'   => 'mail.brandname.com',
              'smtp_port'   => '465',
              'smtp_user'   => 'activation@brandname.com',
              'smtp_pass'   => 'dh4wy3p1c',
              'mailtype'    => 'html',
              'charset'     => 'iso-8859-1',
              'wordwrap'    => TRUE
            ...
            $this->email->from('activation@brandname.com', 'brandname');
            $this->email->to($email);
            $this->email->subject('Verifikasi Email - brandname');
            $message = "<html><head><meta http-equiv='Content-Type' content='text/html; charset=utf-8'></head><body><p><strong>Hallo, $username</strong></p><p>Hanya tinggal 1 langkah lagi untuk bisa bergabung dengan brandname.</p><p>Silahkan mengklik link di bawah ini</p>".$url."<br/><p>Salam Hangat</p><p>brandname Team</p></body></html>";
